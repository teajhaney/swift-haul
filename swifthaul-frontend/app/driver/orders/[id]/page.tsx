...
import { DRIVER_QUEUE, CTA_CONFIG, DRIVER_NEXT_STATUS } from '@/constants/driver-queue';
...
export default function DriverDeliveryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function handleFailed(failReason: FailReason, notes: string) {
    updateStatus(
      { referenceId: id, status: 'FAILED', note: `${failReason}${notes ? `: ${notes}` : ''}` },
    ...
    return (
      <>
        ...
        <div className="max-w-2xl mx-auto px-4 py-5 space-y-4 animate-pulse">
          <div className="h-6 w-48 bg-surface-elevated rounded mx-auto" />
    ...
    return (
      <>
        ...
        <div className="max-w-2xl mx-auto px-4 py-10 text-center">
          ...
          <p className="text-sm font-semibold text-text-primary">Order not found</p>
          <p className="text-xs text-text-muted mt-1">This order may have been removed or you don&apos;t have access.</p>
        </div>
      </>
    );
  }
  ...
  const canShowFailButton = cta !== null && order.status !== 'DELIVERED' && order.status !== 'CANCELLED';
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 pb-32 space-y-4">
        ...
        <div className="text-center py-2">
          <span className={`inline-block px-3 py-1 rounded text-xs font-bold tracking-wider uppercase mb-1 ${STATUS_COLORS[order.status as DriverOrderStatus] ?? 'bg-gray-100 text-gray-600'}`}>
            {order.status.replaceAll('_', ' ')}
          </span>
          <p className="font-mono text-sm text-text-muted">{order.referenceId}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="bg-surface rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <h2 className="text-lg font-bold text-text-primary">{order.recipientName}</h2>
                </div>
                <a
            ...
            <div className="bg-surface rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <h2 className="text-base font-bold text-text-primary">{order.senderName}</h2>
                </div>
                <a
            ...
            <div className="bg-surface rounded-xl border border-border shadow-sm p-5">
              ...
              <p className="text-sm text-text-secondary leading-relaxed">{order.packageDescription}</p>
          ...
          <div className="w-full sm:w-64 shrink-0 space-y-4">
            ...
            <div className="bg-surface rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className={`px-2 py-0.5 rounded text-[11px] font-bold tracking-wider uppercase ${STATUS_COLORS[order.status as DriverOrderStatus] ?? 'bg-gray-100 text-gray-600'}`}>
                  {order.status.replaceAll('_', ' ')}
                </span>
                <span className={`px-2 py-0.5 rounded text-[11px] font-bold tracking-wider uppercase ${PRIORITY_COLORS[order.priority]}`}>
                  {order.priority}
                </span>
              </div>
                ...
                {order.timeline.map((event, idx) => (
                  <div key={idx} className="flex items-start gap-3">
                    ...
                    <div className="pb-1">
                      ...
                      {event.timestamp && (
                        <p className="text-[10px] text-text-muted mt-0.5">{formatTime(event.timestamp)}</p>
                      )}
                    </div>
                  </div>
                ))}
